...
export default function Card() {
  const [userExtension, setExtension] = useState(null);
  ...
  return (
    <div class="container mx-auto max-w-6xl p-4">
      <div class="flex flex-col md:flex-row items-center justify-between mb-4">
        ...
        <div>
          <button
            class="px-8 py-4 border-2 ml-2 mr-2 hover:bg-red-700 text-dark  dark:text-white font-bold rounded-full transition-transform transform-gpu hover:-translate-y-1 hover:shadow-lg"
            onClick={() => setExtension(null)}
          >
            All
          </button>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4 sm:p-1 mt-2">
        {extension.map((item) => {
          if (userExtension && item.isActive) {
            return (
          ...
          } else if (!userExtension && !item.isActive) {
            return (
              <div class="animate-in zoom-in duration-200">
                <div
                  class="ring-1 rounded-lg flex flex-col space-y-2 p-4 break-inside-avoid mb-6 bg-white hover:shadow-md z-0 relative"
